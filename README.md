...
A development tool to reload Python scripts in TS4 without restarting the game.

Based on work by [fetusdip](https://modthesims_info/m/9027427) and [scumbumbo](https://modthesims_info/m/7401825):  
https://modthesims_info/showthread.php?t=534451

## 🧑‍💻 Usage

To reload a script located somewhere in a `Scripts` folder, use part of the file path (without spaces) in the cheat console:
```text
r "C:\Documents\...\Mods\author\Scripts\mod_name\folder\file.py"
```
This will be internally converted to:
```text
r mod_name.folder.file
```

### ↻ Reload the Script Reloader
Either the .ts4script file (recommended) or the .py file can be used.
